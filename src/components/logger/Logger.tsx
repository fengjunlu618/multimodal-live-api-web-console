...
const filters: Record<LoggerFilterType, (log: StreamingLog) => boolean> = {
  ...
  conversations: (log) =>
    ...
    isModelTurn(log.message) ||
    isTurnComplete(log.message),
  audio: (log) => {
    if (isModelTurn(log.message)) {
      return log.message.modelTurn.parts.some(part => part.audio);
    }
    return false;
  },
  text: (log) => {
    if (isModelTurn(log.message)) {
      return log.message.modelTurn.parts.some(part => part.text);
    }
    return isClientContentMessage(log.message);
  },
  image: (log) => {
    if (isModelTurn(log.message)) {
      return log.message.modelTurn.parts.some(part => part.inlineData?.mimeType?.startsWith('image/'));
    }
    return false;
  },
  system: (log) =>
